...
void AvgTimeStr::Loop()
{
   ...
   ////////////////////////
   int num_evt = 0; 
   //TH1F* h_new  = new TH1F(Form("_h_AvgTimeStr_Ch2"), Form("Avg. time structure Ch2"), 1023, 0, 1023);
   //TH1D* h_new  = new TH1D(Form("_h_AvgTimeStr_Ch2"), Form("Avg. time structure Ch2"), 1023, 0, 1023);
   //h_new->Sumw2();
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) 
   {
      ...
      for (int i =0; i < Ch2_ADC->size(); ++i)
      {
         ...
         //h_new->Fill(i+1, Ch2_ADC->at(i));
            h_AvgTime[ich]->Fill(i+1, m_ch_adcs[ich]->at(i));
         }
      }
      num_evt++;
   }//event loop
   //h_new->Scale( (float)(1. / (float) nentries)); 
   for(int ich = 0; ich < 32; ich++){
   //h_new->Fill(i+1, Ch2_ADC->at(i));
      h_AvgTime[ich]->Scale( (float)(1. / (float) nentries));
   }
   printf("Total processed number of events: %lld\n", __tot_evt);
    
}//end Loop function

/*void AvgTimeStr::SetChannels(vector<int> v_chs)
{


}*/
void AvgTimeStr::SetChannels(string confName)
{
...
//   _h_peak = new TH1D(Form("_h_peak"), Form(""), 100000, 0, 10000);
   for(int i = 0; i < 32; ++i){ 
      h_AvgTime[i]  = new TH1D(Form("_h_AvgTimeStr_Ch%d",i), Form("Avg. time structure Ch%d",i), 1023, 0, 1023); h_AvgTime[i]->Sumw2(); h_AvgTime[i]->SetOption("hist");
...
void AvgTimeStr::SetOutputFileName(string outname)
{   
   outfile = outname;
}

void AvgTimeStr::GetVariables()
{
   m_ch_adcs.clear();
   m_ch_adcs[0]=Ch0_ADC; 
   m_ch_adcs[1]=Ch1_ADC; 
